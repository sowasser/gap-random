...
2.  UNDER VM INSTANCES Click on the check mark in the line associated with your username.
...
5.  Paste in the following connection code, substituting `[SERVER NAME]` with the one that has been configured for you (MAYBE DO IN TEXT EDITOR FIRST - NO BRACKETS): `gcloud compute ssh --ssh-flag="-4 -L 8787:localhost:8787" [SERVER NAME] --project=ggn-nmfs-afscdsm-dev-1 --zone=us-east4-c --tunnel-through-iap`
    ...
    It is a good idea to save this connection string to a file on your local machine because you will need to use it every time you connect to the instance.

6.  Say ok to 'keygen' process

7.  When you are done with your session, make sure to turn off the instance by pressing 'STOP' in the menu bar. The connection to the command line and Rstudio will be terminated and the status icon will return to the gray square. This is **very** important for keeping operating costs reasonable.
